...
export async function GET() {
  ...
  const [{ data: risks }, { data: summary }, { data: overdue }] = await Promise.all([
    supabase.from('avr_risks')
      .select(`risk_code, title, inherent_classification, inherent_score, status, treatment_strategy, mrm_reason,
    ...
    <PdfDocument title="MRM Risk Summary Report" author={profile?.full_name ?? 'AVIRA'}>
      <Page size="A4" orientation="landscape" style={shared.page}>
        ...
        <View style={shared.kpiRow}>
          {[
            { label: 'Total Risiko Aktif', value: s?.total_open ?? 0, color: BRAND.navy },
            { label: 'Extreme', value: s?.total_extreme ?? 0, color: BRAND.red },
            { label: 'High', value: s?.total_high ?? 0, color: '#7A4C00' },
            { label: 'Mitigasi Terlambat', value: overdueList.length, color: BRAND.red },
            { label: 'Risiko MRM', value: riskList.length, color: BRAND.navy },
          ].map(k => (
        ...
        <View style={shared.table}>
          ...
          {riskList.length === 0
            ? <View style={[shared.tableRow, { justifyContent: 'center' }]}><Text style={{ fontSize: 8, color: BRAND.muted }}>Tidak ada risiko MRM</Text></View>
            : riskList.map((r, i) => (
