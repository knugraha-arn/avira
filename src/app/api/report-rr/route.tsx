...
export async function GET() {
  ...
  const doc = (
    <PdfDocument title="Risk Register Report" author={profile?.full_name ?? 'AVIRA'}>
      <Page size="A4" orientation="landscape" style={shared.page}>
        ...
        <View style={shared.kpiRow}>
          {[
            { label: 'Total Risiko', value: riskList.length, color: BRAND.navy },
            { label: 'Extreme', value: riskList.filter(r => r.inherent_classification === 'Extreme').length, color: BRAND.red },
            { label: 'High', value: riskList.filter(r => r.inherent_classification === 'High').length, color: '#7A4C00' },
            { label: 'Medium', value: riskList.filter(r => r.inherent_classification === 'Medium').length, color: BRAND.navy },
            { label: 'Low', value: riskList.filter(r => r.inherent_classification === 'Low').length, color: BRAND.navy },
          ].map(k => (
            <View key={k.label} style={shared.kpiCard}>
              <Text style={shared.kpiLabel}>{k.label}</Text>
              <Text style={[shared.kpiValue, { color: k.color }]}>{k.value}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={shared.table}>
          ...
          {riskList.length === 0
            ? <View style={[shared.tableRow, { justifyContent: 'center' }]}><Text style={{ fontSize: 8, color: BRAND.muted }}>Tidak ada risiko aktif</Text></View>
            : riskList.map((r, i) => (
              <View key={r.risk_code} style={[shared.tableRow, i % 2 === 1 ? shared.tableRowAlt : {}]} wrap={false}>
                ...
                <View style={{ width: colW.residual, alignItems: 'center' }}>
                  {r.residual_score > 0
                    ? <><ClassBadge value={r.residual_classification} /><Text style={[shared.tableCellMuted, { marginTop: 2 }]}>{r.residual_score}</Text></>
                ...
                <View style={{ width: colW.status }}><StatusBadgePdf value={r.status} /></View>
                <Text style={[shared.tableCell, { width: colW.review, color: BRAND.muted }]}>{formatDate(r.next_review_date)}</Text>
              </View>
            ))}
        </View>
      </Page>
