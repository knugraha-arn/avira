...
export async function GET() {
  ...
  const doc = (
    <PdfDocument title="High & Extreme Risk Report" author={profile?.full_name ?? 'AVIRA'}>
      <Page size="A4" orientation="landscape" style={shared.page}>
        ...
        <View style={[shared.reportHeaderBox, { borderBottomColor: BRAND.red }]}>
          ...
          <Text style={[shared.complianceBadge, { backgroundColor: '#FFCCCC', color: BRAND.red }]}>ISO 27001 Kl. 8.2</Text>
        </View>

        <View style={shared.alertBox}>
          <Text style={shared.alertText}>
            {`⚠  Laporan ini berisi ${riskList.length} risiko High dan Extreme.${extremeCount > 0 ? `  Terdapat ${extremeCount} risiko Extreme yang harus dibahas di MRM.` : ''}`}
          </Text>
        </View>
        ...
        <View style={shared.table}>
          ...
          {riskList.length === 0
            ? <View style={[shared.tableRow, { justifyContent: 'center' }]}><Text style={{ fontSize: 8, color: BRAND.muted }}>Tidak ada risiko High/Extreme</Text></View>
            : riskList.map((r, i) => (
              <View key={r.risk_code} style={[shared.tableRow, i % 2 === 1 ? shared.tableRowAlt : {}]} wrap={false}>
                ...
                <View style={{ width: colW.title }}>
                  ...
                  {r.existing_control && <Text style={[shared.tableCellMuted, { marginTop: 2 }]}>Kontrol: {r.existing_control}</Text>}
                </View>
                ...
                <View style={{ width: colW.residual, alignItems: 'center' }}>
                  <ClassBadge value={r.residual_classification ?? null} />
                  {r.residual_score > 0 && <Text style={[shared.tableCellMuted, { marginTop: 2 }]}>{r.residual_score}</Text>}
                </View>
                <View style={{ width: colW.treatment }}>
                  <Text style={shared.tableCell}>{r.treatment_strategy ?? '—'}</Text>
                  {r.treatment_notes && <Text style={shared.tableCellMuted}>{r.treatment_notes}</Text>}
                </View>
                ...
                }]}>{formatDate(r.next_review_date)}</Text>
              </View>
            ))}
        </View>
      </Page>
