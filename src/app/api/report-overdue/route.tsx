...
export async function GET() {
  ...
  const colW = { code: '8%', title: '24%', cls: '10%', owner: '13%', target: '9%', late: '8%', progress: '10%', notes: '18%' }

  const doc = (
    <PdfDocument title="Overdue Mitigation Report" author={profile?.full_name ?? 'AVIRA'}>
      <Page size="A4" orientation="landscape" style={shared.page}>
        ...
        <View style={[shared.reportHeaderBox, { borderBottomColor: BRAND.amber }]}>
          ...
          <Text style={[shared.complianceBadge, { backgroundColor: '#FFF0C2', color: '#7A4C00' }]}>ISO 27001 Kl. 9.1</Text>
        </View>

        <View style={[shared.alertBox, { backgroundColor: '#FFFBF0', borderColor: '#FFE0A0' }]}>
          <Text style={[shared.alertText, { color: '#7A4C00' }]}>
            {`⚠  Terdapat ${overdueList.length} mitigasi terlambat per ${now}. Perlu segera ditindaklanjuti.`}
          </Text>
        </View>
        ...
        <View style={shared.table}>
          ...
          {overdueList.length === 0
            ? <View style={[shared.tableRow, { justifyContent: 'center' }]}><Text style={{ fontSize: 8, color: BRAND.muted }}>Tidak ada mitigasi yang terlambat</Text></View>
            : overdueList.map((o: any, i: number) => (
              <View key={`${o.risk_code}-${i}`} style={[shared.tableRow, i % 2 === 1 ? shared.tableRowAlt : {}]} wrap={false}>
                ...
                <Text style={[shared.tableCell, { width: colW.target, color: BRAND.muted }]}>{formatDate(o.target_completion_date)}</Text>
                <Text style={[shared.tableCell, { width: colW.late, color: BRAND.red, fontFamily: 'Helvetica-Bold' }]}>{`+${o.days_overdue} hari`}</Text>
                <Text style={[shared.tableCell, { width: colW.progress }]}>{`${o.progress_percentage}%`}</Text>
                <Text style={[shared.tableCell, { width: colW.notes, color: BRAND.muted }]}>
                  {o.mitigation_notes ? o.mitigation_notes.substring(0, 80) + (o.mitigation_notes.length > 80 ? '...' : '') : '—'}
                </Text>
              </View>
            ))}
        </View>
