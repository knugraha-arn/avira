...
import {
  ...
  X, UserCheck, AlertTriangle, Mail,
} from 'lucide-react'
...
export function UsersClient({ initialUsers, unitKerjaList, currentUserId }: Props) {
  ...
  async function handleInvite() {
    ...
    if (!invEmail.includes('@arranetwork.com')) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-black/50 mt-0.5">
            {users.filter(u => u.is_active).length} aktif · {users.filter(u => !u.is_active).length} nonaktif
          </p>
        </div>
          ...
          <tbody>
            ...
            {filtered.map(u => (
              <tr key={u.id} className={!u.is_active ? 'opacity-50' : ''}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      {u.invited_at && !u.last_login_at && (
                        <span className="text-[10px] text-brand-amber font-medium">● Belum login</span>
                      )}
                    </div>
                  </div>
                </td>
                ...
                <td>
                  <span className={`badge text-[11px] ${roleBadge(u.role)}`}>
                    {u.role === 'risk_manager' ? 'Risk Manager' : u.role.charAt(0).toUpperCase() + u.role.slice(1)}
                  </span>
                </td>
                <td>
