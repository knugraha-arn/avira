...
export default async function EditRiskPage({ params }: Props) {
  ...
  const { data: risk } = await supabase
    .from('avr_risks').select('*').eq('id', id).single()
  if (!risk) notFound()
  if (risk.status === 'Closed') redirect(`/risks/${id}`)

  const [{ data: unitKerjaList }, { data: users }, { data: thirdParties }] = await Promise.all([
    ...
    supabase.from('avr_third_parties').select('id, nama, tipe').eq('is_active', true).order('nama'),
  ])
